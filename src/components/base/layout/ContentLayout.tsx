...
export function ContentLayout({ children, title }: ContentLayoutProps) {
  return (
    <Box className="w-full min-h-[calc(100vh-6rem)] bg-slate-100 items-center">
      <Box
        className={twMerge(
      ...
      <Box className="bg-white w-3/4 max-w-5xl rounded-t-md p-1 mx-auto min-h-[calc(100vh-13rem)]">
        {children}
      </Box>
    </Box>
  );
